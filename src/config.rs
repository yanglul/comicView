...
use crate::{types::{ArchiveFormat, DownloadFormat, ProxyMode}, MyApp};
...
impl Config {
    pub fn new(app: &MyApp) -> anyhow::Result<Self> {
        let app_data_dir = Path::new(".");
    ...
    pub fn save(&self,app: &MyApp) -> anyhow::Result<()> {
        let resource_dir = Path::new(".");
